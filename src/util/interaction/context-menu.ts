...
export = async (client: ExtendedClient, Discord: typeof import("discord.js"), interaction: ContextMenuCommandInteraction) => {
    try {
        ...
        if(interaction.user.id === client.config_main.owner) {
            ...
            console.log(`[interactionCreate] [contextCommand] ${interaction.user.tag} (${interaction.user.id}): /${interaction.commandName} ${interaction.options.data.map((option: any) => option.value ? `${option.name}:${option.value}` : option.name).join(" ")}`);
