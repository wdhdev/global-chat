module.exports = async (user, client, embed) => {
    ...
    if(role.owner) embed.setAuthor({ name: `${user.tag.endsWith("#0") ? `@${user.username}` : user.tag} 👑`, iconURL: user.displayAvatarURL({ format: "png", dynamic: true }), url: `https://discord.com/users/${user.id}` });
}
